  ...
  return shell({
    ...
    children: col(
      ...
      row(
        ...
        row(
          // The row budget is 968: two 176-tall crests (264 wide each) plus this
          // 410 and the two 16 gaps. At 340 the dash printed on top of the left
          // digit; at 440 the crests ran off the card.
          { alignItems: 'center', gap: 20, justifyContent: 'center', width: 410 },
          display(String(game.away.score), { fontSize: 146, letterSpacing: -6, color: winnerIsHome ? C.muted : awayColor }),
          // Drawn, not typed: the display face's dash sits badly against 158px
          // numerals. marginTop drops it onto the digits' optical centre, which
          // sits below the line box's centre.
          h('div', { style: { display: 'flex', width: 46, height: 10, marginTop: 34, borderRadius: 5, backgroundColor: C.muted } }),
          display(String(game.home.score), { fontSize: 146, letterSpacing: -6, color: winnerIsHome ? homeColor : C.muted })
        ),
