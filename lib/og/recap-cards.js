...
import { brandMark, teamLogos } from './assets.js';
...
function finalCard(game, logos, position) {
  const side = (team, align) =>
    col(
      ...
      logos[team.abbr] ? h('img', { src: logos[team.abbr], width: 184, height: 184 }) : null,
  ...
  const scoreRow = (values, color) =>
    row({ alignItems: 'center' }, ...values.map(value =>
      col({ width: 96, alignItems: 'center' }, display(String(value), { fontSize: 30, color }))));
  ...
  return shell({
    ...
    children: col(
      { flexGrow: 1, paddingLeft: 56, paddingRight: 56, paddingTop: 20, paddingBottom: 16, justifyContent: 'space-around' },
      row(
        { alignItems: 'center', gap: 16 },
        side(game.away, 'flex-start'),
        row(
          { alignItems: 'baseline', gap: 26, justifyContent: 'center', width: 340 },
          display(String(game.away.score), { fontSize: 158, letterSpacing: -6, color: winnerIsHome ? C.muted : awayColor }),
          display('–', { fontSize: 58, color: C.muted, letterSpacing: 0 }),
      ...
      periods.length
        ? col(
            ...
            row({ alignItems: 'center', paddingTop: 14 },
              h('div', { style: { display: 'flex', width: 96 } }),
    ...
    children: col(
      ...
      col({ flexGrow: 1 }, ...goals.map((goal, index) =>
        row(
          ...
          logos[goal.team] ? h('img', { src: logos[goal.team], width: 52, height: 52, style: { flexShrink: 0 } }) : null,
    ...
    children: col(
      ...
      col({ paddingLeft: 56, paddingRight: 56, paddingTop: 10, paddingBottom: 18, gap: 8 },
        ...
        row({ alignItems: 'center', gap: 14, paddingTop: 4 },
          logos[game.away.abbr] ? h('img', { src: logos[game.away.abbr], width: 44, height: 44 }) : null,
          label(game.away.abbr, { fontSize: 22, color: awayColor }),
          col({ flexGrow: 1 }),
          label(game.home.abbr, { fontSize: 22, color: homeColor }),
          logos[game.home.abbr] ? h('img', { src: logos[game.home.abbr], width: 44, height: 44 }) : null)),
