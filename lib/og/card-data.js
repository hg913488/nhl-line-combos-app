...
const GAME_TIMEOUT_MS = 6000;

/** Trimmed gamecenter feed for the recap cards, or null if it can't be read. */
export async function recapGame(gameId) {
  if (!/^\d{10}$/.test(String(gameId || ''))) return null;
  try {
    const { buildGame } = await import('../../api/game.js');
    const [pbp, rightRail] = await Promise.all(
      ['play-by-play', 'right-rail'].map(part =>
        fetch(`https://api-web.nhle.com/v1/gamecenter/${gameId}/${part}`, {
          signal: AbortSignal.timeout(GAME_TIMEOUT_MS),
        }).then(response => (response.ok ? response.json() : null))
      )
    );
    if (!pbp) return null;
    return buildGame(pbp, rightRail || {});
  } catch {
    return null;
  }
}
