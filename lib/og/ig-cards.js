...
import { brandMark, teamLogos, playerHeadshots } from './assets.js';
import { C, FONTS, PORTRAIT, h, row, col, label, display, body, rule, divider, chip, fitDisplay, applyTheme } from './theme.js';
...
const lift = (hex, amount = 0.45) => {
  ...
  const mix = channel => {
    ...
    return Math.round(amount >= 0 ? base + (255 - base) * amount : base * (1 + amount));
  };
