...
import { BRAND_MARK, teamLogos, playerHeadshots } from './assets.js';
...
async function slateCard(date) {
  ...
  if (!shown.length) {
    return shell({ date, children: col({ flexGrow: 1, paddingLeft: 56, paddingRight: 56, justifyContent: 'center', gap: 16 },
  ...
  return shell({
    ...
    children: col(
      ...
      col({ flexGrow: 1 }, ...shown.map((game, index) => {
        const awayColor = lift(TEAM_COLORS[game.away] || C.border, 0.35);
        const homeColor = lift(TEAM_COLORS[game.home] || C.border, 0.35);
        return row(
          {
            alignItems: 'center',
            flexGrow: 1,
            ...
            borderTopColor: C.border,
            borderBottomColor: C.border,
            backgroundImage: `linear-gradient(90deg, ${awayColor}1F 0%, rgba(23,25,29,0) 28%, rgba(23,25,29,0) 72%, ${homeColor}1F 100%)`,
          },
          logos[game.away] ? h('img', { src: logos[game.away], width: 56, height: 56, style: { flexShrink: 0 } }) : null,
          ...
          logos[game.home] ? h('img', { src: logos[game.home], width: 56, height: 56, style: { flexShrink: 0 } }) : null,
          col({ flexGrow: 1 }),
...
const lift = (hex, amount = 0.45) => {
  ...
  const mix = channel => Math.round(parseInt(channel, 16) + (255 - parseInt(channel, 16)) * amount);
  return `rgb(${mix(value.slice(0, 2))}, ${mix(value.slice(2, 4))}, ${mix(value.slice(4, 6))})`;
};
...
const shell = ({ children, accent = C.rule, date }) =>
  col(
    { width: PORTRAIT.width, height: PORTRAIT.height, backgroundColor: C.bg },
    rule(accent, 12),
    row(
      ...
      row(
        { alignItems: 'center', gap: 18 },
        h('img', { src: BRAND_MARK, width: 64, height: 48 }),
    ...
    children,
    row(
      { alignItems: 'center', justifyContent: 'space-between', paddingLeft: 56, paddingRight: 56, paddingTop: 26, paddingBottom: 32 },
      label(SITE_TAGLINE, { fontSize: 20, letterSpacing: 1 }),
      label(SITE_LABEL, { fontSize: 20, color: C.accent, letterSpacing: 1 })
    )
  );
...
async function movesCard(date) {
  ...
  if (!moves.length) {
    return shell({ date, accent: C.accent, children: col({ flexGrow: 1, paddingLeft: 56, paddingRight: 56, justifyContent: 'center', gap: 20 },
      ...
      col(
        {
          ...
          height: 596,
          overflow: 'hidden',
          backgroundImage: `radial-gradient(circle at 76% 46%, ${wash} 0%, rgba(23,25,29,0) 60%)`,
        },
        logos[hero.abbr]
          ? h('img', { src: logos[hero.abbr], width: 560, height: 560, style: { position: 'absolute', right: -110, top: 30, opacity: 0.14 } })
          : null,
        heroShot
        ...
        col(
          ...
          display(surnameOf(hero.player), { fontSize: fitDisplay(surnameOf(hero.player), { max: 96, min: 52, perChar: 7.6 }), letterSpacing: -2, lineHeight: 0.95 }),
          headline
            ? row(
            ...
            shots[move.player]
              ? h('div', {
                  style: {
                    ...
                    backgroundColor: `${TEAM_COLORS[move.abbr] || C.surface}`, alignItems: 'flex-end', justifyContent: 'center',
                  },
                }, h('img', { src: shots[move.player], width: 78, height: 78, style: { objectFit: 'contain' } }))
              : logos[move.abbr]
                ? h('img', { src: logos[move.abbr], width: 64, height: 64 })
                : null,
            col(
              { gap: 4, flexGrow: 1 },
...
async function goaliesCard(date) {
  ...
  const season = seasonForDate(new Date(`${date}T12:00:00Z`));
  const [logos, shots] = await Promise.all([
    teamLogos(duels.flatMap(duel => [duel.away.abbr, duel.home.abbr])),
    playerHeadshots(
      duels.flatMap(duel => [
  ...
  const face = (entry, align) => {
    ...
    return row(
      ...
      align === 'right' ? col(
        ...
        label(entry.abbr, { fontSize: 20, color: lift(color, 0.5) })
      ) : null,
      shot
        ? h('div', { style: { display: 'flex', flexShrink: 0, width: 96, height: 96, borderRadius: 48, overflow: 'hidden', backgroundColor: lift(color, 0.15), alignItems: 'flex-end', justifyContent: 'center' } },
            h('img', { src: shot, width: 104, height: 104, style: { objectFit: 'contain' } }))
        : logos[entry.abbr] ? h('img', { src: logos[entry.abbr], width: 84, height: 84, style: { flexShrink: 0 } }) : null,
      align === 'left' ? col(
        { gap: 4, maxWidth: 250 },
        display(surnameOf(entry.goalie || 'TBD').toUpperCase(), { fontSize: fitDisplay(surnameOf(entry.goalie || 'TBD'), { max: 34, min: 19, perChar: 2.2 }), letterSpacing: -0.5, lineHeight: 1 }),
        label(entry.abbr, { fontSize: 20, color: lift(color, 0.5) })
            ...
            col(
              {
                ...
                borderTopStyle: 'solid',
                borderTopColor: C.border,
                backgroundImage: `linear-gradient(90deg, ${lift(TEAM_COLORS[duel.away.abbr] || C.surface, 0.1)}22 0%, rgba(23,25,29,0) 35%, rgba(23,25,29,0) 65%, ${lift(TEAM_COLORS[duel.home.abbr] || C.surface, 0.1)}22 100%)`,
              },
              row(
    ...
    children: col(
      ...
      rows.length
        ? col({ flexGrow: 1, paddingLeft: 56, paddingRight: 56, gap: 0 }, ...rows.map((entry, index) => {
            ...
            const color = lift(TEAM_COLORS[entry.abbr] || C.accent, 0.35);
            return col(
              {
                flexGrow: 1,
                justifyContent: 'center',
              ...
              row(
                ...
                logos[entry.abbr] ? h('img', { src: logos[entry.abbr], width: 52, height: 52 }) : null,
                ...
                display(String(entry.value), { fontSize: 48, color: C.text, letterSpacing: -1 })
              ),
