...
/**
 * Carousel position marker: a dot per slide plus an "03 OF 08" pill, so a card
 * saved or reposted on its own still says where it sat in the set.
 */
export function slideMarker(index, total) {
  ...
  return row(
    { alignItems: 'center', justifyContent: 'space-between', paddingLeft: 56, paddingRight: 56, paddingTop: 10, paddingBottom: 6 },
    h('div', { style: { display: 'flex', width: 150 } }),
    row({ alignItems: 'center', gap: 10 }, ...dots),
    row(
      {
        alignItems: 'center',
        justifyContent: 'center',
